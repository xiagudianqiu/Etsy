...
### 🎨 AI 生图（GPT-Image-2）
- 整合 [awesome-gpt-image-2](https://github.com/EvoLinkAI/awesome-gpt-image-2-API-and-Prompts) 的 624 个精选提示词
- 7 大分类：电商主图 / 广告创意 / 人像摄影 / 海报插画 / UI社媒 / 角色设计 / 对比
- 每个提示词带参数化占位符（如品牌名、颜色），自动生成输入框
- 一键调用 EvoLink GPT-Image-2 API 生成图片
- API Key 用户自备（存 Supabase，仅本人可见）
- 生成历史 + 下载 + 重新生成
